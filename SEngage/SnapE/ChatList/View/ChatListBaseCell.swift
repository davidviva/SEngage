...
public enum ChatCellStatus: Int {
    case Center = 1, Left, Right
}


protocol ChatListBaseCellDelegate {
    func didSelectedLeftButton(index: Int)
    func didSelectedRightButton(index: Int)
}

class ChatCellScrollView: UIScrollView, UIGestureRecognizerDelegate {
...
class ChatListBaseCell: UITableViewCell {
    ...
    var delegate: ChatListBaseCellDelegate?
    
    var tableViewpPanGesture: UIPanGestureRecognizer!
    var cellStatus: ChatCellStatus = .Center
    var cellScrollView: ChatCellScrollView!
    var containtCellView: UIView!
    
    var leftButtonView: ButtonView!
    var rightButtonView: ButtonView!
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
        ...
        cellScrollView = ChatCellScrollView()
    ...
    func scrollViewTapped(gestureRecognizer: UIGestureRecognizer) {
        ...
        for cell in containingTableView.visibleCells {
            let newCell = cell as! ChatListBaseCell
            if newCell != self && newCell.isKindOfClass(ChatListBaseCell.self) {
    ...
    func rightButtonHandler(gestureRecognizer: ButtonTapGestureRecognizer) {
        delegate?.didSelectedRightButton(gestureRecognizer.buttonIndex)
        hiddenButtonsAnimated(true)
    }
    
    func leftButtonHandle(gestureRecognizer: ButtonTapGestureRecognizer) {
    ...
    func contentOffsetForCellStatus(status: ChatCellStatus) -> CGPoint {
    ...
    func updateCellStatus() {
        ...
        for newStatus in [ChatCellStatus.Center, ChatCellStatus.Left, ChatCellStatus.Right] {
...
extension ChatListBaseCell: UIScrollViewDelegate {
